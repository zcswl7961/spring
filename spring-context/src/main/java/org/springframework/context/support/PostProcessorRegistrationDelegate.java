...
final class PostProcessorRegistrationDelegate {
	...
	public static void invokeBeanFactoryPostProcessors(
		...
		if (beanFactory instanceof BeanDefinitionRegistry) {
			...
			// 先处理硬编码注册的后处理器
			/**
			 * 这个就是通过addBeanFactoryPostProcessors（）的方式，代码注入
			 */
			...
			/**
			 * 自动匹配解析出来的BeanDefinitions中类型为BeanDefinitionRegistryPostProcessor.class，
			 */
			...
			for (String ppName : postProcessorNames) {
				if (beanFactory.isTypeMatch(ppName, PriorityOrdered.class)) {  // 先处理那些有顺序的BeanDefinitionRegistryPostProcessors
					/**
					 * 在这里，将配置文件中对应的BeanDefinitionRegistryPostProcessor，实例化对象，调用getBean()方法
					 */
			...
			//将实现了PriorityOrdered的BeanDefinitionRegistryPostProcessor，优先加载到registryProcessors容器中
			registryProcessors.addAll(currentRegistryProcessors);

			/**
			 * 执行从配置文件中解析到的BeanDefinitionRegistryPostProcessor#postProcessBeanDefinitionRegistry 方法
			 */
			...
			for (String ppName : postProcessorNames) {
				//处理那些实现了Ordered接口的BeanDefinitionRegistryPostProcessor，并且，当前的ppName不在之前
				//已经处理了PriorityOrdered之后，加入到实例化的processedBeans容器中
		...
		// 不要在这里初始化FactoryBeans：我们需要保留所有未初始化的常规bean，以便让bean工厂的后处理器应用于它们。
		// Do not initialize FactoryBeans here: We need to leave all regular beans
		// uninitialized to let the bean factory post-processors apply to them!
